...
import { ClipboardList, Utensils, CheckSquare, Plus, Edit, Trash2, Package, Settings, Warehouse, TrendingUp, Sliders, Menu, X, Eye, EyeOff, Sparkles, Home, LogOut, ChevronDown } from 'lucide-react';
...
export default function AdminDashboard({ products, categories, orders, options, onSaveProduct, onDeleteProduct, onUpdateOrderStatus, onDeleteOrder, onSaveOptions, onSaveInventory, onLogout, isSidebarOpen, setIsSidebarOpen }) {
    ...
    return (
        <>
            ...
            <div className="container mx-auto px-4 py-6 animate-fade-in pb-24 transition-colors duration-300">
                {/* Header con título */}
                <div className="flex justify-between items-start md:items-center gap-4 mb-8">
                    <div className="flex items-center gap-3">
                        <div>
                            <h1 className="text-xl md:text-3xl font-extrabold text-slate-900 dark:text-white mb-0.5 md:mb-1 transition-colors duration-300 flex items-center gap-2">
                                <span>Admin</span>
                                <span className="text-rose-600 dark:text-rose-400 hidden xs:inline">Media Luna</span>
                            </h1>
                            <p className="text-slate-500 dark:text-slate-400 text-[10px] sm:text-xs md:text-sm transition-colors duration-300">Gestiona inventario, pedidos y opciones.</p>
                        </div>
                    </div>
                ...
                {activeTab !== 'home' && (
                    <div className="bg-white dark:bg-slate-800/80 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-hidden transition-colors duration-300 backdrop-blur-sm">
                        ...
                        <div className="hidden md:flex border-b border-slate-100 dark:border-slate-700/50 overflow-x-auto transition-colors duration-300">
                            {tabs.map(tab => {
                                const Icon = tab.icon;
                                return (
                                    <button
                                        key={tab.id}
                                        onClick={() => handleTabChange(tab.id)}
                                        className={`flex items-center gap-2 px-6 py-4 font-bold text-sm whitespace-nowrap border-b-2 transition-colors duration-300 ${activeTab === tab.id
                                            ? 'border-rose-500 dark:border-rose-400 text-rose-600 dark:text-rose-400 bg-rose-50/30 dark:bg-rose-900/10'
                                            : 'border-transparent text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-50/30 dark:hover:bg-slate-800/50'
                                            }`}
                                    >
                                        <Icon size={18} />
                                        {tab.label}
