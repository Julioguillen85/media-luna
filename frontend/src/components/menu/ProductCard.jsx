...
export default function ProductCard({ product, cart, onProductClick, removeFromCart, CUSTOMIZABLE_IDS, TRAY_IDS }) {
    const isSelected = cart.some(item => item.id === product.id);
    const isCustomizable = CUSTOMIZABLE_IDS.includes(product.id) || (TRAY_IDS && TRAY_IDS.includes(product.id));
    ...
    return (
        <div className={`glass-panel p-4 rounded-2xl transition-all duration-300 flex items-center gap-4 relative group overflow-hidden ${isSelected && !isCustomizable ? 'ring-2 ring-rose-300 shadow-rose-100 dark:ring-rose-500 dark:shadow-rose-900/50' : 'hover:-translate-y-1'}`}>
            <div className="w-24 h-24 flex-shrink-0 bg-slate-100 dark:bg-slate-800 rounded-2xl overflow-hidden shadow-inner relative cursor-pointer border border-white/70 dark:border-white/10" onClick={() => hasGallery && onProductClick(product, true)}>
                ...
                {(!product.isRental && !product.category?.includes('Renta') && product.productType !== 'RENTAL' && product.category !== 'Muebles') && (
